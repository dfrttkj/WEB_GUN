...
const UNLOCK_PASSWORD = "pleitegeier";

const lockToggleBtn = document.getElementById("lockToggleBtn");
const lockIcon = document.getElementById("lockIcon");
const unlockInputWrap = document.getElementById("unlockInputWrap");
...
function setLockedState(locked) {
  ...
  // Input only visible when locked
  unlockInputWrap.classList.toggle("show", locked);
  if (locked) {
    unlockInput.value = "";
    unlockInput.focus();
  }

  // Blur content + show locked overlay
  contentShell.classList.toggle("is-locked", locked);
  lockedOverlay.classList.toggle("hidden", !locked);

  // Mode popup must not appear while locked
  if (locked) {
    modeOverlay.classList.add("hidden");
  } else {
    // show mode popup only when unlocked (initial flow)
    if (!modeOverlayWasCompleted) {
...
function tryUnlockFromInput() {
  ...
  if (value === UNLOCK_PASSWORD) {
    ...
    unlockInput.value = "";
    unlockInput.placeholder = "Wrong password";
    setTimeout(() => (unlockInput.placeholder = "Password"), 900);
  }
}

// Toggle lock by clicking icon
lockToggleBtn.addEventListener("click", () => {
  if (isUnlocked) setLockedState(true);
  else unlockInput.focus();
});
...
/* Track if mode selection was completed (so it won't pop again) */
let modeOverlayWasCompleted = false;
...
playersTabBtn.addEventListener("click", () => setLeftMode("players"));
...
function syncLivesStepperUI() {
  ...
  livesMinusBtn.disabled = gameSettings.lives <= 1;
  livesPlusBtn.disabled = gameSettings.lives >= 99;
}
...
livesMinusBtn.addEventListener("click", () => setGameSettings({ lives: gameSettings.lives - 1 }));
livesPlusBtn.addEventListener("click", () => setGameSettings({ lives: gameSettings.lives + 1 }));
...
function validateCreate() {
  ...
  for (let i = 0; i < availablePlayerIDs.length; i++) {
    ...
    if (name.length === 0) return `Player ${id} needs a name.`;
  }

  if (currentMode === "tdm") {
    if (teams.length < 2) return "Team Deathmatch requires at least 2 teams.";
  ...
  if (!Number.isFinite(gameSettings.cooldownMs) || gameSettings.cooldownMs < 0) return "Cooldown must be 0 or more.";
  if (gameSettings.cooldownMs > 600000) return "Cooldown is too high.";

  if (!Number.isFinite(gameSettings.lives) || gameSettings.lives < 1) return "Lives must be at least 1.";
  if (gameSettings.lives > 99) return "Lives is too high.";

  if (currentMode === "ffa") {
    if (!Number.isFinite(gameSettings.durationMin) || gameSettings.durationMin < 1) return "Game length must be at least 1 minute.";
    if (gameSettings.durationMin > 999) return "Game length is too high.";
  }
...
// On load: if locked -> show lock overlay, hide mode popup
setLockedState(!isUnlocked);

// If unlocked on load, show mode popup (unless already completed in this session)
if (isUnlocked) {
  modeOverlay.classList.remove("hidden");
}
